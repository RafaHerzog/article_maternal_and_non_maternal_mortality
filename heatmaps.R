...
df_aux_municipios <- read.csv("databases/df_aux_municipios.csv") |>
  mutate(regiao = factor(regiao, levels = c("Norte", "Nordeste", "Centro-Oeste", "Sudeste", "Sul"))) |>
  select(uf = sigla_uf, regiao) |>
  unique() |>
  group_by(regiao) |>
  arrange(uf, .by_group = TRUE) |>
  ungroup()
  
df_obitos_grav_puerp <- read.csv("databases/Obitos_maternos_e_desconsiderados_2010_2021.csv") |>
  filter(idade>= 10 & idade <= 49)

df_tabela <- df_obitos_grav_puerp |>
  mutate(
...
tbl_cross(
  ...
  bold_labels() |>
  modify_caption(
    caption = '<div style = "color: black; text-align: center;"><b>Tabela 5:</b> Tabela cruzada entre as variáveis "faixa etária" e "óbito materno".</div>',
    text_interpret = "html"
  )


df_nascimentos <- read.csv("databases/Nascimentos_muni2010_2021.csv")

df_obitos_heatmap <- df_obitos_grav_puerp |>
  mutate(obitos = 1) |>
  group_by(ano = ano_obito, uf = res_sigla_uf, obito_materno) |>
  summarise(obitos = sum(obitos)) |>
  ungroup() |>
  pivot_wider(
    names_from = obito_materno,
    values_from = obitos,
    values_fill = 0
  ) |>
  clean_names() |>
  mutate(obitos_maternos_e_desconsiderados = nao + sim) |>
  select(ano, uf, obitos_maternos = sim, obitos_maternos_e_desconsiderados)

df_rmm <- left_join(left_join(df_aux_municipios |> select(!regiao), df_nascimentos), df_obitos_heatmap) |>
  mutate(
    rmm_antiga = round(obitos_maternos / nascidos * 100000, 1),
    rmm_nova = round(obitos_maternos_e_desconsiderados / nascidos * 100000, 1),
    mudanca = rmm_nova - rmm_antiga,
    porc_mudanca = rmm_nova / rmm_antiga 
  ) |>
  select(uf, ano, rmm_nova) |>
  pivot_wider(
    names_from = ano,
    values_from = rmm_nova
  ) |>
  as.data.frame()

### Nomeando as linhas dos dataframes com os nomes dos respectivos estados
row.names(df_rmm) <- df_rmm$uf

### Retirando a coluna com o nome das UFs e transformando os dataframes em matrizes
df_rmm <- df_rmm |>
  dplyr::select(!uf) |>
  as.matrix() 

### Definindo a paleta de cor do mapa de calor
cores <- colorRampPalette(brewer.pal(8, "YlOrRd"))(20)
cores <- colorRampPalette(brewer.pal(8, "YlOrBr"))(20)
cores <- colorRampPalette(brewer.pal(8, "OrRd"))(20)

### Criando os mapas de calor
df_aumento_rmm <-left_join(left_join(df_aux_municipios |> select(!regiao), df_nascimentos), df_obitos_heatmap) |>
  mutate(
    rmm_antiga = round(obitos_maternos / nascidos * 100000, 1),
    rmm_nova = round(obitos_maternos_e_desconsiderados / nascidos * 100000, 1),
    mudanca = round(rmm_nova - rmm_antiga, 1),
    porc_mudanca = round((rmm_nova / rmm_antiga * 100) - 100, 1),
    coluna = glue::glue("{mudanca} ({porc_mudanca}%)")
  ) |>
  select(uf, ano, coluna) |>
  pivot_wider(
    names_from = ano,
    values_from = coluna
  ) |>
...
pheatmap(
  df_rmm, 
  cluster_rows = FALSE, cluster_cols = FALSE, color = cores, angle_col = 45, border_color = "#F8F8F8",
  main = "Mapa de calor da RMM das UFs brasileiras quando considerados os óbitos não declarados",
  display_numbers = round(df_rmm, 2),
  fontsize_number = 9,
  number_color = "black"
)
...
teste <- df_obitos_grav_puerp |>
  group_by(ano_obito, obito_materno) |>
  mutate(obitos = 1) |>
  summarise(obitos = sum(obitos)) |>
  pivot_wider(
    names_from = obito_materno,
  ...
  ungroup() |>
  mutate(
    obitos_maternos_e_desconsiderados = nao + sim
  ) |>
  select(obitos_maternos = sim, obitos_maternos_e_desconsiderados)

nascimentos <- df_nascimentos |>
  summarise(nascidos = sum(nascidos)) |>
  pull(nascidos)

poisson.test(c(sum(teste$obitos_maternos_e_desconsiderados), sum(teste$obitos_maternos)), rep(length(teste$obitos_maternos), 2))
library(rateratio.test)
rateratio.test(c(sum(teste$obitos_maternos_e_desconsiderados), sum(teste$obitos_maternos)), c(nascimentos, nascimentos), alternative = "greater")
